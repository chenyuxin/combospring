 ...
 * 支持 < 和 <= 两种比较操作
 */
public class LessThan extends QueryCondition {
     ...
     */
    public LessThan(String fieldName, Object fieldValue, String paramName) {
        super();
     ...
     */
    public LessThan(String fieldName, Object fieldValue, String paramName, boolean isEqual) {
        super();
        this.fieldName = fieldName;
        this.fieldValue = fieldValue;
        this.paramName = paramName;
    ...
    public <T> Predicate getPredicate(Root<T> root, CriteriaBuilder cb) {
        ...
        if (this.isEqual) {
            // 小于等于 <=
            return cb.lessThanOrEqualTo(root.get(this.fieldName), (Comparable) this.fieldValue);
        } else {
            // 小于 <
            return cb.lessThan(root.get(this.fieldName), (Comparable) this.fieldValue);
        }
    }
