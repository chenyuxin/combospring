...
public class QueryConditionCommonLike extends QueryCondition {
	...
	public String getSqlString(String where) {
		...
		sqBuilder.append(StringPool.SPACE).append(where).append(StringPool.SPACE);
		sqBuilder.append(this.fieldName);
		if (this.not) {// 使用not like语句
			if (null == this.fieldValue) {
				sqBuilder.append(" is not null ");
			} else {
				sqBuilder.append(" not like :").append(this.paramName).append(StringPool.SPACE);
			}
		} else {
			if (null == this.fieldValue) {
				sqBuilder.append(" is null ");
			} else {
				sqBuilder.append(" like :").append(this.paramName).append(StringPool.SPACE);
			}
		}
	...
	public void setParamMap(Map<String, Object> params) {
		if (null != this.fieldValue) {// 筛选条件值为null的不用添加参数变量
			...
			params.put(this.paramName, likeValue);
		}
	}
	...
	public <T> Predicate getPredicate(Root<T> root, CriteriaBuilder cb) {
		if (this.not) {// 使用not like语句
			if (null == this.fieldValue) {
				...
				return cb.notLike(root.get(this.fieldName).as(String.class), String.valueOf(this.fieldValue));
			}
		} else {
			if (null == this.fieldValue) {
				return cb.isNull(root.get(this.fieldName));
			} else {
				return cb.like(root.get(this.fieldName).as(String.class), String.valueOf(this.fieldValue));
			}
		}
	}
