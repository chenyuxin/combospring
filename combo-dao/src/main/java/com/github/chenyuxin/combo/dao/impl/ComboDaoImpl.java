package com.github.chenyuxin.combo.dao.impl;
...
import com.github.chenyuxin.combo.dao.resource.DaoResource;
import com.github.chenyuxin.combo.dao.resource.anotation.Table;
import com.github.chenyuxin.combo.dao.resource.anotation.TableUtil;
import com.github.chenyuxin.combo.dao.resource.options.DaoOptions;
...
import com.github.chenyuxin.combo.dao.resource.sql.CommonSql;
...
public class ComboDaoImpl implements ComboDao{
	...
	private String useTable(Map<String, Object> paramMap, String dataSourceName, String ctrlSql,boolean isThrowException) {
		...
		return DaoUtil.daoMessage(DaoUtil.useTable_SUCCESS_MESSAGE, rows);
	}
	...
	private String saveObjList(List<?> list, TableType tableType, String dataSourceName,boolean isThrowException) {
		...
		daoResource.putTableTypeCache(tableType,dataSourceName);
		return DaoUtil.daoMessage(DaoUtil.saveObj_SUCCESS_MESSAGE,list.size());//添加作业数据量的返回
	}
	...
	private String updateList(List<?> list, String[] fieldNameByIds, TableType tableType, String dataSourceName,boolean isThrowException) {
		...
		//daoResource.checkQureyDataCacheMap(dataSourceName,tableType.getTableName());//TODO 扩展redis缓存
		daoResource.putTableTypeCache(tableType,dataSourceName);
		return DaoUtil.daoMessage(DaoUtil.updateObj_SUCCESS_MESSAGE,list.size());//更新作业数据量的返回
	}
	...
	private String saveOrUpdateList(List<?> list, String[] fieldNameByIds, TableType tableType, String dataSourceName,boolean isThrowException){
		...
		//daoResource.checkQureyDataCacheMap(dataSourceName,tableType.getTableName());//TODO 扩展redis缓存
		daoResource.putTableTypeCache(tableType,dataSourceName);
		return DaoUtil.daoMessage(DaoUtil.mergeObj_SUCCESS_MESSAGE,list.size());//更新作业数据量的返回
	}
	...
	private String deleteObjSingle(Object object, String dataSourceName,boolean isThrowException){
		...
		String[] fieldNameByIds = TableUtil.getIdsbyObj(object.getClass());
		Table table = object.getClass().getAnnotation(Table.class);
		TableType tableType = TableType.c(table.name());
	...
	public <T> PageBean<T> selectObj(int currentPage, int pageSize, Class<T> clazz, Object... daoOptionsO) {
		DaoOptions daoOptions = new DaoOptions(daoOptionsO);
		TableType tableType = TableType.c(clazz.getAnnotation(Table.class).name());
