...
public interface CommonDao {
	 ...
	 * @return
	 */
	Page<Map<String, Object>> selectObjMap(int currentPage, int pageSize, List<String> attributeNames, TableType tableType, Object... daoOptions);
	 ...
	 * @return
	 */
	<T> Page<T> selectObj(int currentPage, int pageSize, Class<T> clazz, Object... daoOptions);
	 ...
	 * @return
	 */
	<T> Page<T> selectObj(String sql, int currentPage, int pageSize, Class<T> clazz,
			Object... daoOptions);
	 ...
	 * @param clazz
	 * @param daoOptions 设置参数 {paramMap 条件查询参数驼峰会转下划线  ,dataSourceName 其它数据源在spring配置文件bean的id,queryConditions 自定义通用查询条件}
	 * @param tableType 模型表名(选填) 若填入时使用填入的表名 	
	 * @param paramMap 条件查询参数(驼峰会转下划线)
	 * @param dataSourceName 其它数据源在spring配置文件bean的id
	 * @param queryConditions 自定义通用查询条件
	 * @param DaoEnumOptions 在执行出错时抛出RuntimeException异常，不配置此项，默认返回失败消息或空值。
	 * @return
	 */
	<T> List<T> selectObj(Class<T> clazz, Object... daoOptions);
	
	/**
	 * 查询获取单个对象，实体模型用类封装<br>
	 * 当查询结果不为单个对象时，返回null
	 * @param clazz
	 * @param daoOptions 设置参数 {paramMap 条件查询参数驼峰会转下划线  ,dataSourceName 其它数据源在spring配置文件bean的id,queryConditions 自定义通用查询条件}
	 * @param paramMap 条件查询参数(驼峰会转下划线)
	 ...
	 * @return
	 */
	Long getRecords(TableType tableType, Object... daoOptions);
